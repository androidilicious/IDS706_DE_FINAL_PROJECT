...
def analyze_delivery_performance():
    ...
    df = (
        orders.join(reviews, on="order_id", how="inner")
        .filter(
    ...
    print(f"\n📈 INTERPRETATION:")
    print(
        f"   - For each additional day of delivery, review score changes by {slope:.4f} points"
    )
    print(
        f"   - {abs(r_value**2)*100:.2f}% of review score variation explained by delivery time"
    )

    # Aggregate stats
    delivery_stats = (
        df.group_by(pl.col("delivery_days").cast(pl.Int32))
        .agg(
            [
...
def analyze_product_categories():
    ...
    best_rated = category_stats.filter(
        pl.col("items_sold") >= 100
    ).sort(  # At least 100 sales
        "avg_review", descending=True
    )
